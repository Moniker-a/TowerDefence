...
void SystemManager::update(EntityManager &_em)
{
    ...
    {
        for (Entity iEntity = 0; iEntity < _em.get_component_list_size(); iEntity++) //For each entity (and deleted entity).
        {
            boost::dynamic_bitset<> entityComponentsMask = _em.get_entity_mask(iEntity); //Store entity's component mask (a bitset which indicates which components an entity has).
            ...
            if (systemComponentsMask == (systemComponentsMask & entityComponentsMask)) //If the current entity contains all the required components for the current system to operate on.
            {
                ...
                currentSystemPair.second->update(iEntity, _em);
            }
        }
    }
}
