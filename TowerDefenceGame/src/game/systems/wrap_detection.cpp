...
namespace System
{
    ...
    WrapDetection::WrapDetection(EntityManager* _em, EventBus* _eb) : BaseSystem(_em, _eb)
    {
        ...
        systemMask.set(World::get_registry().get_ID<Component::BoundingBox>());
    }

    //System logic here... Updates an entity.
    void WrapDetection::update(Entity _entityID)//, EntityManager& _em)
    {
        auto bBox = em->get_component<Component::BoundingBox>(_entityID).lock(); //Get pointer to the entity's bounding box.
        auto position = em->get_component<Component::Position>(_entityID).lock(); //Get pointer to the entity's position.
